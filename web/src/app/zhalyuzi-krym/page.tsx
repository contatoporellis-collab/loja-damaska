...
import { landingZhalyuziKrym as landing } from "@/data/landing-zhalyuzi-krym";
...
/** WhatsApp с преднастроенным сообщением под крымский заказ. */
const krymWhatsappHref =
  `${contacts.whatsappHref}?text=` +
  encodeURIComponent(
    "Здравствуйте! Хочу заказать жалюзи/рулонные шторы с доставкой в Крым. Пришлю размеры и фото окна.",
  );

export default function ZhalyuziKrymLandingPage() {
  const jsonLd = {
    ...
    areaServed: ["Симферополь", "Севастополь", "Ялта", "Евпатория", "Керчь"],
        ...
        <section className="sun-wash">
          <Container className="grid gap-10 py-12 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:gap-12 lg:py-20">
            ...
            <LeadForm
              ...
              {...leadFormProps("zhalyuzi-krym-hero")}
            />
          </Container>
        </section>
            ...
            <Container className="absolute inset-x-0 bottom-0">
              <div className="flex flex-col gap-2 pb-7">
                <span className="font-display text-lg font-bold text-bone sm:text-2xl">
                  Изготовим по вашим размерам — доставим в Крым
                </span>
                <span className="text-sm text-bone/80 sm:text-base">
                  Собственная фабрика с 2011 года · Симферополь · Севастополь ·
                  Ялта · весь Крым
                </span>
              </div>
            </Container>
            ...
            <div className="sun-wash relative overflow-hidden rounded-[var(--radius-xl)] border border-line bg-surface">
              <div className="grid gap-10 p-7 sm:p-10 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
                ...
                <LeadForm
                  ...
                  subtitle={landing.hero.formSubtitle}
                  {...leadFormProps("zhalyuzi-krym-final")}
                />
              </div>
            </div>
          </Container>
